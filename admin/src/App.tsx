...
function Settings({ tab, notify }: { tab: "project" | "theme" | "billing"; notify: (message: string) => void }) {
  ...
  if (!data) return <Loading />;
  if (tab === "billing") return <><Card title="Current plan" description={`${data.plan_label} · ${data.max_projects} project(s) · status ${data.status}`} /> {data.public_beta && <div className="notice" style={{ margin: "16px 0", padding: "12px 16px", borderRadius: "8px", background: "var(--accent-soft)", borderLeft: "3px solid var(--accent)" }}>🚀 <strong>100% Free Public Beta:</strong> Hosted Cloud is currently free with all Pro features unlocked. Early adopters receive lifetime perks.</div>}<div className="plans">{(data.plans || []).map((plan: Data) => <Card key={plan.id} title={plan.label} className={plan.id === data.plan ? "current-plan" : ""}><strong className="price">{data.public_beta ? (plan.id === "free" ? "$0 Forever" : "$0 / Public Beta") : plan.price_try ? `${plan.price_try.amount_cents / 100} TRY` : "Free"}</strong><ul>{(plan.features || []).map((feature: string) => <li key={feature}>{feature}</li>)}</ul><Button primary={plan.id !== data.plan}>{plan.id === data.plan ? "Current plan" : data.public_beta ? "Included in Beta" : "Upgrade"}</Button></Card>)}</div></>;
...
export function App() {
  ...
  const switchProject = async (id: string) => { try { document.cookie = `sooauth_project=${encodeURIComponent(id)}; path=/; max-age=31536000; SameSite=Lax`; await request(`/projects/${id}/select`, { method: "POST" }); location.reload(); } catch (e) { notify((e as Error).message); } };
